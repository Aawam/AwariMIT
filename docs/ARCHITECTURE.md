...
ADR-009: `MarketDataProvider`, `HistoryRequest`, `MarketHistoryResult`, and normalized full `PriceBar` types define the provider boundary. Provider response handling must normalize before data reaches analytics, risk, screening, or UI. No provider implementation is active because V0.4 qualification did not establish a reproducible automated multi-ticker IDX OHLCV route. Reviewed capture ingestion remains the explicit, provider-independent path.
